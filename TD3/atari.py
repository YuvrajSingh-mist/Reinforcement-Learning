...
class Config:
    # Experiment settings
    exp_name = "TD3-Atari-Breakout"
    seed = 42
    env_id = "BreakoutNoFrameskip-v4"
    policy_noise= 0.3
    low = -2.0
    high = 2.0  # Action space limits for Breakout
    # Training parameters
...
class QNet(nn.Module):
    def __init__(self, state_space, action_space):
        ...
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(64 * 2 * 2, 512)
        self.fc2 = nn.Linear(action_space, 512)
        # self.q_value = nn.Linear(512, action_space)
        
        self.fc3 = nn.Linear(1024, 1024)
        self.reduce = nn.Linear(1024, 512)  # Output a single Q-value
...
def evaluate(model, device, run_name, num_eval_eps = 10, record = False):
    ...
    for eps in range(num_eval_eps):
        ...
        while not done:
            ...
            # with torch.no_grad():
            with torch.no_grad():
                action, _ = model.get_action(torch.tensor(obs, device=device, dtype=torch.float32).unsqueeze(0))
...
actor_net = ActorNet(env.observation_space.shape[0], env.action_space.n).to(device)

q1_network = QNet(env.observation_space.shape[0], env.action_space.n).to(device)
q2_network = QNet(env.observation_space.shape[0], env.action_space.n).to(device)

target_actor_net = ActorNet(env.observation_space.shape[0], env.action_space.n).to(device)
target_q1_network = QNet(env.observation_space.shape[0], env.action_space.n).to(device)
target_q2_network = QNet(env.observation_space.shape[0], env.action_space.n).to(device)
...
for step in tqdm(range(args.total_timesteps)):
    ...
    with torch.no_grad():  # No need to track gradients for environment interactions
        action, log_probs = actor_net.get_action(torch.tensor(obs, device=device, dtype=torch.float32).unsqueeze(0))
    ...
    if step > args.learning_starts:
        ...
        with torch.no_grad():
            next_actions, next_log_prob = target_actor_net.get_action(data.next_observations)
        ...
        if step % args.train_frequency == 0:
            ...
            actions = actor_net.get_action(data.observations)
            action_values = -q1_network(data.observations, actions).mean()  # Maximize Q-value for next actions
            actor_optim.zero_grad()
            action_values.backward()
            actor_optim.step()
    ...
    if step > args.learning_starts and step % 500 == 0:
        # Save model
