...
for step in tqdm(range(args.total_timesteps)):
    ...
    if step > args.learning_starts:
        ...
        # returns = torch.tensor(returns, device=device, dtype=torch.float32)
            ...
            policy_loss = -q_network(data.observations.to(torch.float32), actions.to(torch.float32)).mean()  # Maximize Q-value for next actions
...
if args.use_wandb:
    train_video_path = f"videos/final_MountainCarContinuous-v0.mp4"
