...
parser.add_argument("--total_timesteps", type=int, default=1000, help="Total timesteps for training")
# MATCHING num_envs from custom script
parser.add_argument("--n_envs", type=int, default=16, help="Number of parallel environments") 
parser.add_argument("--run", type=int, default=1,help="Run number for the experiment")
args = parser.parse_args()

run_name = f"NeatRL-Benchmark-Custom-PPO-seed-{args.seed}-{args.env_id}"

# ===== CONFIGURATION =====
class Config:
    # Experiment settings
    # exp_name = "PPO(GAE)-HalfCheetah-v1-Vectorized"
    seed = args.seed
    env_id = args.env_id
...
run = wandb.init(
    ...
    # group=group_name,
    name=f"NeatRL-Benchmark-Custom-PPO-seed-{args.seed}-{args.env_id}",
    config=vars(args),
...
def make_env(env_id, seed, idx, gamma, eval_mode=False, render_mode=None):
    def thunk():
...
def evaluate(envs, model, device, gamma, num_eval_eps=30, record=False, render_mode=None):
    # Create a "raw" evaluation environment without the normalization wrappers
    eval_env = make_env(Config.env_id, Config.seed, 0, gamma, eval_mode=True, render_mode=render_mode)()
...
if __name__ == "__main__":
    ...
    for update in tqdm(range(1, num_updates + 1), desc="Training Updates"):
        ...
        if update % 100 == 0:
            episodic_returns, _ = evaluate(envs, actor_network, device, args.gamma)
    ...
    if args.capture_video:
        ...
        episodic_returns, eval_frames = evaluate(envs, actor_network, device, args.gamma, record=True, num_eval_eps=30, render_mode='rgb_array')
