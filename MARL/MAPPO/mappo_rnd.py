...
class PredictorNet(nn.Module):
    def __init__(self, observation_dim=18):
        ...
        self.out = layer_init(nn.Linear(32, 32), std=0.1)

    def forward(self, x):
        # x shape: [batch_size, obs_dim] or [obs_dim]
        if len(x.shape) == 1:
            x = x.unsqueeze(0)
...
class TargetNet(nn.Module):
    def __init__(self, observation_dim=18):
        ...
        self.network = nn.Sequential(
            ...
            layer_init(nn.Linear(64, 32)),
            nn.Tanh(),
            layer_init(nn.Linear(32, 32), std=0.1)
        )
    ...
    def forward(self, x):
        ...
        if len(x.shape) == 1:
            x = x.unsqueeze(0)
        return self.network(x)  # Target network outputs features directly
