...
class Config:
    ...
    total_timesteps = 10_000_000 * 2  # Standard     metric for vectorized training
...
def evaluate(actor_networks, device, seed, num_eval_eps=10, record=False):
    eval_env = simple_tag_v3.env(render_mode="rgb_array", continuous_actions=True)
    env = ss.pad_observations_v0(eval_env)
...
if __name__ == "__main__":
    ...
    # Create the base (padded) PettingZoo environment
    env = make_env(env_id=args.env_id, seed=args.seed, idx=0, run_name=run_name)

    # --- NEW: Identify agent roles and create role-based networks ---
    possible_agents = env.possible_agents
    num_agents = len(possible_agents)
    agent_roles = [agent_id.split('_')[0] for agent_id in possible_agents]
    unique_roles = sorted(list(set(agent_roles))) # e.g., ['adversary', 'agent']

    # Dictionaries to hold the networks and optimizers for each role
    actor_networks = {role: Actor(obs_shape, action_dim).to(device) for role in unique_roles}
    critic_networks = {role: Critic(obs_shape).to(device) for role in unique_roles}
    optimizers = {
        role: optim.Adam(
            list(actor_networks[role].parameters()) + list(critic_networks[role].parameters()),
            lr=args.lr, eps=1e-5
        ) for role in unique_roles
    }

    print(f"Agent roles in fixed order: {agent_roles}")
    ...
    for actor in actor_networks.values():
        actor.train()

    for critic in critic_networks.values():
        critic.train()
    ...
    for update in tqdm(range(1, num_updates + 1), desc="Training Updates"):
        ...
        for role in unique_roles:
            optimizers[role].param_groups[0]['lr'] = lr
            if args.anneal_lr:
                for param_group in optimizers[role].param_groups:
                    param_group['lr'] = lr
        ...
        for step in range(0, args.max_steps):
            ...
            with torch.no_grad():
                for agent_idx in range(num_agents):
                    # Determine the role for this index (e.g., idx 0 -> 'adversary')
                    role = agent_roles[agent_idx]
                    
                    agent_obs = next_obs[masks[agent_idx]]
                    
                    # Use the correct network for this role
                    value = critic_networks[role](agent_obs)
                    action, logprob, entropy = actor_networks[role].get_action(agent_obs)
        ...
        for epoch in range(args.PPO_EPOCHS):
            ...
            for start in range(0, len(b_game_inds), game_minibatch_size):
                ...
                for agent_idx in range(args.num_agents):
                    ...
                    current_ext = critic_networks[role](mb_obs)
                ...
                # for agent_idx in range(args.num_agents):
                    ...
                    # Calculate losses
                    new_log_probs, entropy = actor_networks[role].evaluate_get_action(mb_obs, mb_actions)
                ...
                # for agent_idx in range(args.num_agents):
                    ...
                    nn.utils.clip_grad_norm_(actor_networks[role].parameters(), 0.5)

                    # Critic network logging and clipping
                    grad_norm_dict = {}
                    total_norm = 0
                    for name, param in critic_networks[role].named_parameters():
                        if param.grad is not None:
                            param_norm = param.grad.data.norm(2)
