...
predictor_network = PredictorNet(env.observation_space.shape[0], env.action_space.n).to(device)
target_network = TargetNet(env.observation_space.shape[0], env.action_space.n).to(device)
_optim = optim.Adam(list(actor_network.parameters()) + list(critic_network.parameters()) + list(predictor_network.parameters()), lr=args.lr)
# actor_optim = optim.Adam(actor_network.parameters(), lr=args.lr)
# critic_optim = optim.Adam(critic_network.parameters(), lr=args.lr)
# predictor_optim = optim.Adam(predictor_network.parameters(), lr=args.lr)

for param in target_network.parameters():
    param.requires_grad = False  # Freeze target network parameters
...
for step in tqdm(range(args.episodes)):
    ...
    # while not done:
        ...
        pred_val = predictor_network(torch.tensor(new_obs, device=device, dtype=torch.float32).unsqueeze(0)).squeeze()
        target_val = target_network(torch.tensor(new_obs, device=device, dtype=torch.float32).unsqueeze(0)).squeeze()
        intrinsic_reward = F.mse_loss(pred_val, target_val).item()
        intrinsic_rewards.append(intrinsic_reward)
        ext_value, int_value = critic_network(torch.tensor(obs, device=device, dtype=torch.float32).unsqueeze(0))
        ...
        #     dones.append(False)
        # states.append(obs)
    # rewards_to_tensor = torch.tensor(rewards, device=device, dtype=torch.float32)
    # intrinsic_rewards_to_tensor = torch.tensor(intrinsic_rewards, device=device, dtype=torch.float32)
    # Calculate the total rewards by combining extrinsic and intrinsic rewards
    # rewards = rewards_to_tensor + intrinsic_rewards_to_tensor
    # rewards = rewards.tolist()  # Convert to list for further processing
    
    # returns = []
    curr_observation = obs # The state after the last step
    #Extrinsic reward
    # intrinsic_rew = target_network(torch.tensor(obs, device=device, dtype=torch.float32).unsqueeze(0)).squeeze().item()
    # total_rewards = rewards + 
    # Use this logic after the fixed-size rollout
    int_returns = []
    ext_returns= []
    if dones[-1]: # If the rollout ended on a terminal state
        bootstrap_scalar_int = 0.0
        bootstrap_scalar_ext = 0.0
    else:
        with torch.no_grad():
            bootstrap_scalar_ext, bootstrap_scalar_int = critic_network(torch.tensor(curr_observation, device=device, dtype=torch.float32).unsqueeze(0)).squeeze()
            bootstrap_scalar_ext = bootstrap_scalar_ext.item()
            bootstrap_scalar_int = bootstrap_scalar_int.item()
            # bootstrap_scalar_ext = target_network(torch.tensor(curr_observation, device=device, dtype=torch.float32).unsqueeze(0)).squeeze().item()

    int_gt_next_state = bootstrap_scalar_int
    ext_gt_next_state = bootstrap_scalar_ext
    for reward_at_t, done_at_t in zip(reversed(rewards), reversed(dones)):
        if done_at_t:
            int_gt_next_state = 0.0
            ext_gt_next_state = 0.0
        rt_int = reward_at_t + args.gamma * int_gt_next_state
        rt_ext = reward_at_t + args.gamma * ext_gt_next_state
        int_returns.insert(0, rt_int)
        ext_returns.insert(0, rt_ext)
        # Update the next state values
        int_gt_next_state = rt_int
        ext_gt_next_state = rt_ext

    int_returns = torch.tensor(int_returns, device=device, dtype=torch.float32)
    ext_returns = torch.tensor(ext_returns, device=device, dtype=torch.float32)
    int_values = torch.stack(int_values).squeeze()
    ext_values = torch.stack(ext_values).squeeze()

    # Calculate advantages and detach immediately to avoid gradient issues
    int_advantages = (int_returns - int_values).detach()  # Calculate advantages and detach
    int_advantages = (int_advantages - int_advantages.mean()) / (int_advantages.std() + 1e-8)  # Normalize advantages
    int_returns = int_returns.detach()  # Detach returns

    ext_advantages = (ext_returns - ext_values).detach()  # Calculate advantages and detach
    ext_advantages = (ext_advantages - ext_advantages.mean()) / (ext_advantages.std() + 1e-8)  # Normalize advantages
    ext_returns = ext_returns.detach()  # Detach returns
    
    advantages = int_advantages + args.EXT_COEFF *  ext_advantages  # Combine intrinsic and extrinsic advantages
    # Log episode returns
    # if "episode" in info:
    #     print(f"Step={step}, Return={info['episode']['r']}")
    ...
    states_visited_rewards = target_network(states_tensor)  # Get rewards for all states at once
    # PPO Update epochs
    for epoch in range(args.PPO_EPOCHS):
        ...
        intrinsic_rewards = predictor_network(states_tensor)  # Get intrinsic rewards for all states at once
        # print(f"Intrinsic rewards shape: {intrinsic_rewards.shape}, States visited rewards shape: {states_visited_rewards.shape}")
        intrinsic_loss = torch.nn.functional.mse_loss(intrinsic_rewards, states_visited_rewards)
        ...
        ext_current_values, int_current_values = critic_network(states_tensor).squeeze()  # Get current values for all states at once
        # print(f"Current values shape: {current_values.shape}, Returns shape: {returns.shape}")
        int_critic_loss = VALUE_COEFF * torch.nn.functional.mse_loss(int_current_values, int_returns)
        ext_critic_loss = VALUE_COEFF * torch.nn.functional.mse_loss(ext_current_values, ext_returns) 
        critic_loss  = int_critic_loss + ext_critic_loss
        
        
        loss = surrogate_loss + critic_loss + intrinsic_loss - args.ENTROPY_COEFF * entropy.mean()  
