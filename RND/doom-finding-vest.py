...
class Config:
    # Experiment settings
    exp_name = "PPO-RND-Vectorized-Viz"
    seed = 42
    ...
    for _ in tqdm(range(num_eval_eps), desc="Evaluating"):
        ...
        while not done:
            ...
            if display:
                safe_display(obs)
            with torch.no_grad():
                action, _, _ = model.get_action(torch.tensor(obs, device=device, dtype=torch.float32).unsqueeze(0), deterministic=True)
...
if __name__ == "__main__":
    ...
    if args.capture_video:
        ...
        episodic_returns, eval_frames = evaluate(actor_network, device, run_name, num_eval_eps=5, record=True, render_mode='rgb_array')
