    ...
    for update in tqdm(range(1, num_updates + 1), desc="Training Updates"):
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.max_steps)):
                # gt_next_state = values_storage[t + 1]
                # 4. Calculate the return at the current step 't'.
                # This is your 'rt = reward_at_t + args.gamma * gt_next_state'.
                delta = rewards_storage[t] + args.gamma * gt_next_state * (1-dones_storage[t]) * args.GAE - values_storage[t]
                ...
                gt_next_state = values_storage[t+1] * (1.0 - dones_storage[t]) # If done at t, the next gt is 0
