...
if __name__ == '__main__':
    ...
    env = VecFrameStack(env, 4)  # Stack 4 frames to match the custom script
    env = VecMonitor(env, f"logs/{run_name}")  # Monitor for logging
    ...
    eval_env = make_atari_env(args.env_id, n_envs=1, seed=args.seed, vec_env_cls=DummyVecEnv)
    eval_env = VecFrameStack(eval_env, 4)  # Stack 4 frames for evaluation
    env = VecMonitor(env, f"logs/{run_name}/monitor")  # Monitor for training env
